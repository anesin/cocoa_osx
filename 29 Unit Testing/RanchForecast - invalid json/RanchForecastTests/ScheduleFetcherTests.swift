...
class ScheduleFetcherTests: XCTestCase {
    ...
    func testCreateCourseFromInalidDictionary() {
        let invalidCourseDict = NSMutableDictionary(dictionary: Constants.validCourseDict)

        for (key, value) in invalidCourseDict {
            invalidCourseDict.removeObjectForKey(key)
            let course: Course! = fetcher.courseFromDictionary(invalidCourseDict)
            XCTAssertNil(course)
            invalidCourseDict[key as! String] = value
        }
    }
