...
class Document: NSDocument {
    ...
    func addConstraints() {
        ...
        var buttonAdd: NSButton?
        var buttonRemove: NSButton?
        for v in view.subviews {
            if v.isKindOfClass(NSScrollView) {
                scrollView = v as? NSScrollView
            } else if v.isKindOfClass(NSButton) {
        ...
        let viewsDict = ["scrollView": scrollView!, "buttonAdd": buttonAdd!, "buttonRemove": buttonRemove!]
       
        let verticalScrollView = NSLayoutConstraint.constraintsWithVisualFormat("V:|-[scrollView(>=181)]-|", options: [], metrics: nil, views: viewsDict)
        NSLayoutConstraint.activateConstraints(verticalScrollView)

        let horizontalScrollViewAdd = NSLayoutConstraint.constraintsWithVisualFormat("|-[scrollView(>=344)]-[buttonAdd(115)]-|", options: [], metrics: nil, views: viewsDict)
        NSLayoutConstraint.activateConstraints(horizontalScrollViewAdd)
        
        let verticalButtons = NSLayoutConstraint.constraintsWithVisualFormat("V:|-[buttonAdd]-[buttonRemove]", options: [], metrics: nil, views: viewsDict)
        NSLayoutConstraint.activateConstraints(verticalButtons)

        let widthButtons2 = NSLayoutConstraint.constraintsWithVisualFormat("[buttonRemove(==buttonAdd)]", options: [], metrics: nil, views: viewsDict)
        NSLayoutConstraint.activateConstraints(widthButtons2)
        
        let horizontalScrollViewRemove = NSLayoutConstraint.constraintsWithVisualFormat("[scrollView]-[buttonRemove]", options: [], metrics: nil, views: viewsDict)
        NSLayoutConstraint.activateConstraints(horizontalScrollViewRemove)
