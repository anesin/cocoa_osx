...
class ViewController: NSViewController {
    ...
    @IBAction func togglePinging(sender: NSButton) {
        ...
        if let task = task {
            ...
            notificationCenter.addObserver(self, selector: #selector(ViewController.receiveDataReadyNotification(_:)), name: NSFileHandleReadCompletionNotification, object: fileHandle)
            notificationCenter.addObserver(self, selector: #selector(ViewController.receiveTaskTerminatedNotification(_:)), name: NSTaskDidTerminateNotification, object: task)
            task.launch()
