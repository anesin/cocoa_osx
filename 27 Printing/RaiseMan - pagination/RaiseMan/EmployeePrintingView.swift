...
class EmployeePrintingView: NSView {
    ...
    override func knowsPageRange(range: NSRangePointer) -> Bool {
        ...
        linesPerPage = Int(pageRect.height / lineHeight) - 1  // -1: for pagination
    ...
    override func drawRect(dirtyRect: NSRect) {
        ...
        let pagination = String(format: "- %d -", currentPage + 1)
        let size = pagination.sizeWithAttributes(textAttributes)
        let x = pageRect.minX + (pageRect.width - size.width)/2
        let paginationRect = NSRect(x: x, y: pageRect.maxY - lineHeight, width: size.width, height: lineHeight)
        pagination.drawInRect(paginationRect, withAttributes: textAttributes)
    }
