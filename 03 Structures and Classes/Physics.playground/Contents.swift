...
struct Vector: CustomStringConvertible {  // Swift 2
    var x: Double
    var y: Double
...
let twoGsAdd = gravity + gravity
twoGsAdd.x
twoGsAdd.y
let twoGsMulti = gravity * 2.0
twoGsMulti.x
twoGsMulti.y
...
let simul = Simulation()
let rocket = Rocket(thrust: 10.0, thrustTime: 60.0)
simul.addParticle(rocket)
while simul.particles.count > 0 && simul.time < 500 {
    simul.tick(1.0)
}
