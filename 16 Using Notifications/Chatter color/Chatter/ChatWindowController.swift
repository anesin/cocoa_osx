...
class ChatWindowController: NSWindowController {
    ...
    @IBAction func send(sender: NSButton) {
        ...
        if let message = message where message.characters.count > 0 {
            ...
            if name.characters.count > 0 && username != name {
                username = name
            }
            let userInfo = [ChatWindowControllerMessageKey: username + " - " + message]
            let notificationCenter = NSNotificationCenter.defaultCenter()
    ...
    func receiveDidSendMessageNotification(note: NSNotification) {
        ...
        if note.object as! ChatWindowController == self {
            let logLine = NSAttributedString(string: message, attributes: [NSForegroundColorAttributeName: NSColor.blueColor()])
