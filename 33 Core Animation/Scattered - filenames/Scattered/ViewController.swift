...
class ViewController: NSViewController {
        ...
        while let url = directoryEnumerator.nextObject() as? NSURL {
            ...
            if let isDirectory = isDirectoryValue as? NSNumber where isDirectory.boolValue == false {
                ...
                if let image = image {
                    ...
                    presentImage(thumbImage, contentsOfURL: url)
    ...
    func presentImage(image: NSImage, contentsOfURL url: NSURL) {
        ...
        let filenameLayer = configureTextLayer(url.lastPathComponent)
    ...
    func configureTextLayer(text: String?) -> CATextLayer {
        let textLayer = CATextLayer()
        textLayer.anchorPoint = CGPoint.zero
        ...
        textLayer.shadowOffset = NSMakeSize(0, 0)
        textLayer.borderColor = NSColor.whiteColor().CGColor
